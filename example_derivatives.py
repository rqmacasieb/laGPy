...
def example_1d_derivatives():
    ...
    h = 1e-10  # Small step size for finite differences
    
    for i in range(n_test):
        # Create a function that evaluates the GP at a specific point
        def gp_at_point(x):
            # Reshape x to match the expected input format
            x_reshaped = np.array([[x]])
            # Get GP prediction at this point
            gp_result = laGP(x_reshaped, X_train, Z_train, start=20, end=50, method="alc", lite=True, verb=1, compute_derivatives=False)
            return gp_result['mean'][0]
        ...
        fd_derivatives[i] = (gp_at_point(x_plus) - gp_at_point(x_minus)) / (2 * h)
